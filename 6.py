...
def make_vectors(cmd):
	aa = [(0,0), (0,0)]
	for x in cmd:
		aa.append( vector (aa[-1], x) )
	return aa
	
line_a = LineString(make_vectors(a))
line_b = LineString(make_vectors(b))

# if line_a.intersects(line_b):
int_pt = line_a.intersection(line_b)
def distance(a, pt):
	r = 0
	line_a = LineString(make_vectors(a))
	while pt.intersection(line_a):
		a = a[:-1]
		
		line_a = LineString(make_vectors(a))
	line_a = LineString(make_vectors(a)+[(pt.x, pt.y)])
	r = line_a.length
	return r
	
def distances(a, b, pt):
	r = 0
	r = distance(a, pt) + distance(b, pt)
	return r

for p in int_pt[1:]:
	print p, distances(a,b,p)